...
class MCPredictionFA(ApproximationAgent):
    ...
    def __init__(self, env, discount_factor, transformer, policy, estimator, feature_converter):
        ...
        self.state_shape = self.env.observation.shape
    ...
    def learn(self, episodes):
        ...
        estimate_derivative = self.estimator.get_derivative()
        ...
        for i in range(episodes):
            ...
            for i in range(len(states)):
                ...
                state_feature = featurize(states[-i-1])
                state_transformed = transform(state_feature)
                state_visits[state_transformed] += 1
                self.alpha = 1/state_visits[state_transformed]
                error = greturn - estimate(state_feature)
